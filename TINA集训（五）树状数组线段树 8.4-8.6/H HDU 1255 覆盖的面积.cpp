...
{
    double x;//竖线的横坐标
    double y_down, y_up;//竖线的上下端点的y坐标
    int flag;//矩形左侧还是右侧
}line[N];
struct node
{
...
double Insert_query(int rt, double x, double l, double r, int flag)
{
    ...
    if (node[rt].flag)//叶子结点，只有到最小区间更新
    {
        if (node[rt].cover>1)//超过1次
        {
...
int main()
{
    ...
    while (t--)
    {
        ...
        for (int i = 0; i<n; i++)
        {
            ...
            line[cnt].y_down = y1;
            line[cnt].y_up = y2;
            line[cnt].flag = 1;//矩形左边
            y[++cnt] = y2;
            line[cnt].x = x2;
            line[cnt].y_down = y1;
            line[cnt].y_up = y2;
            line[cnt].flag = -1;//矩形右边
        }
        sort(y, y + cnt + 1);//把所有y坐标（横线）按y从小到大排列
        sort(line, line + cnt + 1, cmp);//把所有竖线按x从小到大排列
        build(1, 0, cnt);
        double area = 0;
        for (int i = 0; i <= cnt; i++)
        {
